...
\end{align*}
More formally, we can integrate the values (for succinctness, we define $c(a)$ as the normalizing constant of a Dirichlet distribution with shape $a$) and we image that $X)i \in \{0,1\}^K$ is one-hot encoded as we did in the previous part, with $X_{i,k}$ as the $k$-th element of $X_i$:
\begin{align*}
